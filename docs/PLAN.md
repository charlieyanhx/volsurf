...
4. **Zeliade's 0 ≤ a box is not a no-arbitrage condition** and degrades real SPY fits 18× (RMSE 1.657 vp
   vs 0.093 vp on SPY 2023-11-15 30 DTE; Vogt's own arbitrage example has a < 0). The fitter constrains
   `w_min = a + bσ√(1−ρ²) ≥ 0` and `b(1+|ρ|) ≤ 2` only. Zeliade's (m, σ) reduction is the initialiser:
   41×41 grid with an unconstrained 3-parameter linear inner solve, then one constrained polish —
   0.14 s per slice and the same optimum as 36-start SLSQP (1.2–1.9 s).
...
| `svi.py` | `SVIParams(a,b,rho,m,sigma)`, `SVISlice(params, T)` with `w, dw, d2w, g, min_total_variance, iv(k)`; `LEE_BOUND = 2.0`; JW conversions `to_jw / from_jw` (GJ Lemma 3.2); `from_ssvi(theta, rho, phi)` (a = θ(1−ρ²)/2, b = θφ/2, m = −ρ/φ, σ = √(1−ρ²)/φ); `fit_svi(k, w, T, weights=None, grid=(41,41)) -> SVIFit(params, objective, rmse_vol, max_abs_vol, n, converged, wall_time, wing_slopes=(b(1−ρ), b(1+ρ)), starts)`; `fit_svi_multistart(...)` = the 36-start SLSQP reference kept for the bench. Weights enter the objective as `w·r²`, not `(w·r)²`. |
| `arb.py` | `Evidence` enum {`analytic`, `numerical_scan`}; `butterfly(slice: SVISlice, k_lo, k_hi, margin=None, n=2001) -> ButterflyReport(ok, worst_g, worst_k, n_inside, n_outside, k_range_checked, asymptote_left, asymptote_right)`; `calendar(slices: list[(T, SVISlice)], k_grid) -> CalendarReport(ok, crossings, worst_gap, worst_k, pair)`; `quote_level(slice, F, D) -> QuoteLevelReport(butterfly_raw_mid, butterfly_within_band, calendar_raw, calendar_within_band, n_triplets)` using ask/bid/ask convexity; `repair_jw(params, T)` = GJ §5.1, opt-in, never called by the fitter; `check_arbitrage(fits) -> ArbitrageReport` aggregating all three with `notes`. |
| `weights.py` | `select(slice, F, D, otm_only=True, min_bid_ticks=2, tick=0.01, max_rel_spread=0.2, k_max=None) -> (mask, Ledger)`; `spread_weights(iv_bid, iv_ask, floor=1e-4)` = 1/(iv_ask−iv_bid)² floored; `vega_weights(F, K, T, sigma)`. |
...
- Chain: crossed quote rejected; duplicate key rejected; SPX and SPXW on the same date are different slices; T rule (AM vs PM) pinned to the day.
- Readers: synthetic Cboe-shaped JSON round-trips; mztrading date rule on a synthetic frame with an expired row.

Real-data numbers (private, README): SPY 2021-06-15 (calm), 2025-04-08 (stress), 2020-03-16 (documented
SVI-shape failure, RMSE 1.1–2.0 vp expected), 2023-11-15 (regression day: 30 DTE RMSE 0.093 vp /
59.6 % inside bid/ask, 65 DTE 0.026 vp / 79.0 %, 93 DTE 0.054 vp / 69.0 %); mztrading 2026-08-10 close
for SPX (roots separated), SPY, AAPL, NVDA, XOM; QQQ/IWM one day each. Wording: "static-arbitrage
